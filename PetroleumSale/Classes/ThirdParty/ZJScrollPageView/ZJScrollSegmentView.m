...
static CGFloat const xGap = 5.0+5.0;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            if (self.segmentStyle.scrollLineWidth) {//
                coverW = self.segmentStyle.scrollLineWidth;
                coverX = coverX+ (firstLabel.zj_width - coverW) * 0.5;
            }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    if (_currentIndex == _oldIndex && taped) {
        if (_segmentStyle.tapTitleOneMoreTimes == YES) {
            //多次点击title 只把响应事件传出去
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.scrollLineWidth) {
                    weakSelf.scrollLine.zj_width = self.segmentStyle.scrollLineWidth;
                    weakSelf.scrollLine.zj_centerX = currentTitleView.zj_centerX;
                }
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
...
//    oldTitleView.font =  [UIFont fontWithName:@"PingFangSC-Regular" size:15];
//     currentTitleView.font = [UIFont fontWithName:@"PingFangSC-Semibold" size:15];
//    NSArray *titleViews = [self.segmentView valueForKey:@"titleViews"];
//    for (int i = 0;i<titleViews.count;i++) {
//        if (i== self.currentIndex) {
//            UIFont *currentFont = [UIFont fontWithName:@"PingFangSC-Semibold" size:15];
//            ZJTitleView *titleCure  = titleViews[self.currentIndex];
//            UILabel *labelCurret =  [titleCure valueForKey:@"label"];
//            labelCurret.font= currentFont;
//        }else{
//            UIFont *font = [UIFont fontWithName:@"PingFangSC-Regular" size:15];
//            ZJTitleView *title  = titleViews[self.preIndex];
//            UILabel *label =  [title valueForKey:@"label"];
//            label.font= font;
//        }
//    }
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.scrollLineWidth) {
                ...
                self.scrollLine.zj_centerX = oldTitleView.zj_centerX + xDistance * progress;
            }
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
