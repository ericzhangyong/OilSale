...
//body参数请求
static NSDictionary *sg_httpHeaderBodys = nil;
...
static NSTimeInterval sg_timeout = defaultTimeOut;
static BOOL sg_shoulObtainLocalWhenUnconnected = NO;
//设置body参数
static BOOL sg_needSetBodyField = NO;
//默认传param
static BOOL sg_needSetParmaField = YES;
static NSSet *sg_parametersInURISet = nil;
...
+(void)setConfigHeaderNeedParam:(BOOL)needParma neddBody:(BOOL)needBody{
    
    sg_needSetParmaField = needParma;
    sg_needSetBodyField = needBody;
}

+(void)setRequestParametersInURISet:(NSSet *)parametersInURI{

    sg_parametersInURISet = parametersInURI;
}
...
static inline NSString *cachePath() {
  return [NSHomeDirectory() stringByAppendingPathComponent:@"Library/funmily/HYBNetworkingCaches"];
}
...
+ (void)cancelRequestWithTask:(HYBURLSessionTask *)task
{
    @synchronized(self) {
        [task cancel];
        [[self allTasks] removeObject:task];
...
+ (void)configCommonHttpRequestBody:(NSDictionary *)httpBodys {
    sg_httpHeaderBodys = httpBodys;
}
...
+ (HYBURLSessionTask *)uploadWithImageData:(NSData *)imageData
                                   url:(NSString *)url
...
+ (HYBURLSessionTask *)uploadWithImages:(NSMutableArray *)images
                                    url:(NSString *)url
                               filename:(NSString *)filename
                                   name:(NSString *)name
                               mimeType:(NSString *)mimeType
  ...
  switch (sg_requestType) {
    case kHYBRequestTypeJSON: {
      manager.requestSerializer = [AFJSONRequestSerializer serializer];
        //自己添加这些请求类型是param请求 除了这些的其他类型是Body参数请求
        if (sg_parametersInURISet==nil) {
            sg_parametersInURISet  = [NSSet setWithObjects:@"GET", @"HEAD", @"DELETE",@"PUT",nil];
        }
        manager.requestSerializer.HTTPMethodsEncodingParametersInURI = sg_parametersInURISet;
      break;
    }
...
+ (void)logWithFailError:(NSError *)error url:(NSString *)url params:(id)params {
  ...
  if ([error code] == NSURLErrorCancelled) {
    ...
    //HYBAppLog(error.debugDescription);
}
