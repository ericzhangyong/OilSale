...
// 项目打包上线都不会打印日志，因此可放心。
#ifdef DEBUG
#define HYBAppLog(s, ...) fprintf(stderr,"[%s in line %d] ===============> %s",[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __LINE__, [[NSString stringWithFormat:s, ##__VA_ARGS__] UTF8String]);
#else
#define HYBAppLog(s, ... )
#endif

#define defaultTimeOut 1000.0
#define shortTimeOut 6.0
...
/**
 ...
 @param needParma param传参类型
 @param needBody body传参类型
 */
+ (void)setConfigHeaderNeedParam:(BOOL)needParma neddBody:(BOOL)needBody;

/**
 设置请求支持拼接的请求方法类型 比如
 [NSSet setWithObjects:@"GET", @"HEAD", @"DELETE",@"PUT",nil]
 其他的类型请求方法为body
 @param parametersInURI NSSet类型
 */
