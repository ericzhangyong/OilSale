...
//
//  Created by zhangyong on 2019/10/29.
...
static CGFloat const KDefaultToastHideDelay     =   2.5;    //  默认2.5s后消失


@interface BaseProgressHUD ()<MBProgressHUDDelegate>
{
...
+ (BaseProgressHUD *)sharedProgressHUD {
    static BaseProgressHUD *ffProgressHUD = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        ffProgressHUD = [[BaseProgressHUD alloc] init];
    });
...
- (void)showProgressHUDWithTitle:(NSString *)title
    ...
    _mbProgressHUD.bezelView.backgroundColor = [color_010101 colorWithAlphaComponent:0.7f];
...
- (void)toastTitle:(NSString *)title {
    if ([BaseVerifyUtils isNullOrSpaceStr:title]) {
        return;
    }
...
- (MBProgressHUD *)createToastHUD:(UIView *)pView
    ...
    hud.bezelView.backgroundColor = [color_010101 colorWithAlphaComponent:0.7f];
...
-(void)showHUDWithProgress:(CGFloat)pro
{
    ...
    if (!_progressBarHUD) {
        ...
        _progressBarHUD.bezelView.backgroundColor = [color_010101 colorWithAlphaComponent:0.7f];
